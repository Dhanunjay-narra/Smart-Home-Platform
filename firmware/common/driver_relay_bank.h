#ifndef DRIVER_RELAY_BANK_H
#define DRIVER_RELAY_BANK_H

/**
 * @file driver_relay_bank.h
 * @brief Optocoupled 8-Channel SPDT Power Relay Controller with Interlock
 * @copyright 2026 Dhanunjay Narra. All Rights Reserved.
 */

#include <stdint.h>
#include <stdbool.h>

#ifdef __cplusplus
extern "C" {
#endif

typedef enum {
    DRIVER_RELAY_BANK_STATUS_OK = 0,
    DRIVER_RELAY_BANK_STATUS_ERROR = -1,
    DRIVER_RELAY_BANK_STATUS_BUSY = -2,
    DRIVER_RELAY_BANK_STATUS_TIMEOUT = -3
} driver_relay_bank_status_t;

typedef struct {
    uint8_t channel;
    uint32_t baudrate;
    bool is_initialized;
    uint32_t rx_byte_count;
    uint32_t tx_byte_count;
    uint32_t error_count;
} driver_relay_bank_handle_t;

driver_relay_bank_status_t driver_relay_bank_init(driver_relay_bank_handle_t *handle);
driver_relay_bank_status_t driver_relay_bank_read(driver_relay_bank_handle_t *handle, uint8_t *buffer, uint16_t len);
driver_relay_bank_status_t driver_relay_bank_write(driver_relay_bank_handle_t *handle, const uint8_t *data, uint16_t len);
driver_relay_bank_status_t driver_relay_bank_self_test(driver_relay_bank_handle_t *handle);

#ifdef __cplusplus
}
#endif

#endif // DRIVER_RELAY_BANK_H
