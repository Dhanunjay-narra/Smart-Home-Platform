#ifndef HAL_PWM_TIMER_H
#define HAL_PWM_TIMER_H

/**
 * @file hal_pwm_timer.h
 * @brief Hardware Abstraction Layer for High-Resolution Motor & LED PWM Timers
 * @copyright 2026 Dhanunjay Narra. All Rights Reserved.
 */

#include <stdint.h>
#include <stdbool.h>

#ifdef __cplusplus
extern "C" {
#endif

typedef enum {
    HAL_PWM_TIMER_STATUS_OK = 0,
    HAL_PWM_TIMER_STATUS_ERROR = -1,
    HAL_PWM_TIMER_STATUS_BUSY = -2,
    HAL_PWM_TIMER_STATUS_TIMEOUT = -3
} hal_pwm_timer_status_t;

typedef struct {
    uint8_t channel;
    uint32_t baudrate;
    bool is_initialized;
    uint32_t rx_byte_count;
    uint32_t tx_byte_count;
    uint32_t error_count;
} hal_pwm_timer_handle_t;

hal_pwm_timer_status_t hal_pwm_timer_init(hal_pwm_timer_handle_t *handle);
hal_pwm_timer_status_t hal_pwm_timer_read(hal_pwm_timer_handle_t *handle, uint8_t *buffer, uint16_t len);
hal_pwm_timer_status_t hal_pwm_timer_write(hal_pwm_timer_handle_t *handle, const uint8_t *data, uint16_t len);
hal_pwm_timer_status_t hal_pwm_timer_self_test(hal_pwm_timer_handle_t *handle);

#ifdef __cplusplus
}
#endif

#endif // HAL_PWM_TIMER_H
