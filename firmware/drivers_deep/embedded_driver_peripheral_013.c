/**
 * @file embedded_driver_peripheral_013.c
 * @brief Implementation for Peripheral Driver 013
 */

#include "embedded_driver_peripheral_013.h"
#include <stdio.h>

int driver_peripheral_013_init(driver_peripheral_013_t *dev) {
    if (!dev) return -1;
    dev->peripheral_id = 13;
    dev->base_address = 0x40000000 + (13 * 0x1000);
    dev->transaction_count = 0;
    dev->error_count = 0;
    printf("[Peripheral 013] Initialized at 0x%08X\n", dev->base_address);
    return 0;
}
