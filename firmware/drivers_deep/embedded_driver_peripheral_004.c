/**
 * @file embedded_driver_peripheral_004.c
 * @brief Implementation for Peripheral Driver 004
 */

#include "embedded_driver_peripheral_004.h"
#include <stdio.h>

int driver_peripheral_004_init(driver_peripheral_004_t *dev) {
    if (!dev) return -1;
    dev->peripheral_id = 4;
    dev->base_address = 0x40000000 + (4 * 0x1000);
    dev->transaction_count = 0;
    dev->error_count = 0;
    printf("[Peripheral 004] Initialized at 0x%08X\n", dev->base_address);
    return 0;
}
