"""
Smart Home Platform — Automation Rule: DynamicDemandResponseRule
Description: Curtails non-critical loads (pool pump, EV charging, water heater) during peak grid tariff spikes
"""
...
class DynamicDemandResponseRuleEvaluationContext(BaseModel):
    rule_id: str = "dynamic_demand_response"
    description: str = "Curtails non-critical loads (pool pump, EV charging, water heater) during peak grid tariff spikes"
...
class DynamicDemandResponseRule:
    """Rule engine processor with AST evaluation, threshold hysterisis, and safety guards."""
    def __init__(self):
        self.context = DynamicDemandResponseRuleEvaluationContext()
    ...
    def execute_action_pipeline(self, telemetry_snapshot: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "actions_dispatched": [
                {"target": "system", "action": "OPTIMIZE_STATE", "reason": "Curtails non-critical loads (pool pump, EV charging, water heater) during peak grid tariff spikes"}
            ]
        }
