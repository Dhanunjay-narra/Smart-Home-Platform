"""
Smart Home Platform — Automation Rule: KitchenCooktopExhaustAutoSpeedRule
Description: Modulates range hood exhaust fan speed based on cooktop current sensor and particulate density
"""
...
class KitchenCooktopExhaustAutoSpeedRuleEvaluationContext(BaseModel):
    rule_id: str = "kitchen_cooktop_exhaust_auto_speed"
    description: str = "Modulates range hood exhaust fan speed based on cooktop current sensor and particulate density"
...
class KitchenCooktopExhaustAutoSpeedRule:
    """Rule engine processor with AST evaluation, threshold hysterisis, and safety guards."""
    def __init__(self):
        self.context = KitchenCooktopExhaustAutoSpeedRuleEvaluationContext()
    ...
    def execute_action_pipeline(self, telemetry_snapshot: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "actions_dispatched": [
                {"target": "system", "action": "OPTIMIZE_STATE", "reason": "Modulates range hood exhaust fan speed based on cooktop current sensor and particulate density"}
            ]
        }
