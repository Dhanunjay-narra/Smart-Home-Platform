"""
Smart Home Platform — Automation Rule: ServerRackThermalFailsafeRule
Description: Triggers auxiliary split AC and shuts down non-essential compute when lab rack temp exceeds 38C
"""
...
class ServerRackThermalFailsafeRuleEvaluationContext(BaseModel):
    rule_id: str = "server_rack_thermal_failsafe"
    description: str = "Triggers auxiliary split AC and shuts down non-essential compute when lab rack temp exceeds 38C"
...
class ServerRackThermalFailsafeRule:
    """Rule engine processor with AST evaluation, threshold hysterisis, and safety guards."""
    def __init__(self):
        self.context = ServerRackThermalFailsafeRuleEvaluationContext()
    ...
    def execute_action_pipeline(self, telemetry_snapshot: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "actions_dispatched": [
                {"target": "system", "action": "OPTIMIZE_STATE", "reason": "Triggers auxiliary split AC and shuts down non-essential compute when lab rack temp exceeds 38C"}
            ]
        }
