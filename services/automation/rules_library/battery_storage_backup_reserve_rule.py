"""
Smart Home Platform — Automation Rule: BatteryStorageBackupReserveRule
Description: Locks 30% battery reserve when local weather radar indicates severe thunderstorm warning
"""
...
class BatteryStorageBackupReserveRuleEvaluationContext(BaseModel):
    rule_id: str = "battery_storage_backup_reserve"
    description: str = "Locks 30% battery reserve when local weather radar indicates severe thunderstorm warning"
...
class BatteryStorageBackupReserveRule:
    """Rule engine processor with AST evaluation, threshold hysterisis, and safety guards."""
    def __init__(self):
        self.context = BatteryStorageBackupReserveRuleEvaluationContext()
    ...
    def execute_action_pipeline(self, telemetry_snapshot: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "actions_dispatched": [
                {"target": "system", "action": "OPTIMIZE_STATE", "reason": "Locks 30% battery reserve when local weather radar indicates severe thunderstorm warning"}
            ]
        }
