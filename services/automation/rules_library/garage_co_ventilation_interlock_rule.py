"""
Smart Home Platform — Automation Rule: GarageCoVentilationInterlockRule
Description: Activates high-CFM exhaust fan when garage CO exceeds 25 ppm or vehicle engine start detected
"""
...
class GarageCoVentilationInterlockRuleEvaluationContext(BaseModel):
    rule_id: str = "garage_co_ventilation_interlock"
    description: str = "Activates high-CFM exhaust fan when garage CO exceeds 25 ppm or vehicle engine start detected"
...
class GarageCoVentilationInterlockRule:
    """Rule engine processor with AST evaluation, threshold hysterisis, and safety guards."""
    def __init__(self):
        self.context = GarageCoVentilationInterlockRuleEvaluationContext()
    ...
    def execute_action_pipeline(self, telemetry_snapshot: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "actions_dispatched": [
                {"target": "system", "action": "OPTIMIZE_STATE", "reason": "Activates high-CFM exhaust fan when garage CO exceeds 25 ppm or vehicle engine start detected"}
            ]
        }
