"""
Smart Home Platform — Automation Rule: PoolSolarHeatingDiverterRule
Description: Directs solar thermal loop valves to pool heat exchanger when rooftop solar temp > pool water temp
"""
...
class PoolSolarHeatingDiverterRuleEvaluationContext(BaseModel):
    rule_id: str = "pool_solar_heating_diverter"
    description: str = "Directs solar thermal loop valves to pool heat exchanger when rooftop solar temp > pool water temp"
...
class PoolSolarHeatingDiverterRule:
    """Rule engine processor with AST evaluation, threshold hysterisis, and safety guards."""
    def __init__(self):
        self.context = PoolSolarHeatingDiverterRuleEvaluationContext()
    ...
    def execute_action_pipeline(self, telemetry_snapshot: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "actions_dispatched": [
                {"target": "system", "action": "OPTIMIZE_STATE", "reason": "Directs solar thermal loop valves to pool heat exchanger when rooftop solar temp > pool water temp"}
            ]
        }
