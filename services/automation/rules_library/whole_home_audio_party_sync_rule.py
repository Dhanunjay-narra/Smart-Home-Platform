"""
Smart Home Platform — Automation Rule: WholeHomeAudioPartySyncRule
Description: Groups all multi-room audio zones and synchronizes ambient RGB strip lighting to beat detection
"""
...
class WholeHomeAudioPartySyncRuleEvaluationContext(BaseModel):
    rule_id: str = "whole_home_audio_party_sync"
    description: str = "Groups all multi-room audio zones and synchronizes ambient RGB strip lighting to beat detection"
...
class WholeHomeAudioPartySyncRule:
    """Rule engine processor with AST evaluation, threshold hysterisis, and safety guards."""
    def __init__(self):
        self.context = WholeHomeAudioPartySyncRuleEvaluationContext()
    ...
    def execute_action_pipeline(self, telemetry_snapshot: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "actions_dispatched": [
                {"target": "system", "action": "OPTIMIZE_STATE", "reason": "Groups all multi-room audio zones and synchronizes ambient RGB strip lighting to beat detection"}
            ]
        }
