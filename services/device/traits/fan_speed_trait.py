"""
Smart Home Platform Trait: FanSpeedCapability
Description: 4-Speed Capacitor & BLDC Ceiling Fan Speed Controller Trait
"""
...
class FanSpeedCapabilityConfig(BaseModel):
    trait_id: str = "fan_speed"
    display_name: str = "Fan Speed"
...
class FanSpeedCapabilityState(BaseModel):
    trait: str = "fan_speed"
    raw_value: Any = None
...
class FanSpeedCapabilityController:
    """Enterprise Controller handling state transitions and safety validation for FanSpeedCapability."""
    def __init__(self, device_id: str = 'dev-node-001'):
        self.device_id = device_id
        self.config = FanSpeedCapabilityConfig()
        self.state = FanSpeedCapabilityState()
