"""
Smart Home Platform Trait: GarageDoorCapability
Description: Motorized Sectional Overhead Door Trait with Obstacle Optical Sensor
"""
...
class GarageDoorCapabilityConfig(BaseModel):
    trait_id: str = "garage_door"
    display_name: str = "Garage Door"
...
class GarageDoorCapabilityState(BaseModel):
    trait: str = "garage_door"
    raw_value: Any = None
...
class GarageDoorCapabilityController:
    """Enterprise Controller handling state transitions and safety validation for GarageDoorCapability."""
    def __init__(self, device_id: str = 'dev-node-001'):
        self.device_id = device_id
        self.config = GarageDoorCapabilityConfig()
        self.state = GarageDoorCapabilityState()
