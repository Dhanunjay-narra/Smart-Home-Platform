"""
Smart Home Platform Trait: PirMotionCapability
Description: Dual-Element Pyroelectric Infrared (PIR) Motion Detection Trait
"""
...
class PirMotionCapabilityConfig(BaseModel):
    trait_id: str = "pir_motion"
    display_name: str = "Pir Motion"
...
class PirMotionCapabilityState(BaseModel):
    trait: str = "pir_motion"
    raw_value: Any = None
...
class PirMotionCapabilityController:
    """Enterprise Controller handling state transitions and safety validation for PirMotionCapability."""
    def __init__(self, device_id: str = 'dev-node-001'):
        self.device_id = device_id
        self.config = PirMotionCapabilityConfig()
        self.state = PirMotionCapabilityState()
